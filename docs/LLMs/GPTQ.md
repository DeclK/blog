...
1. 证明计算子 Hessian 逆等价于 Cholesky decomposition 子矩阵$L_FL_F^T$
    ...
    $$
    \mathbf{H}^{-1} = L L^T = \begin{bmatrix}
    l_{11} & 0 \\
    l_{21} & L_{22}
    \end{bmatrix}
    \begin{bmatrix}
...
1. 无需重复计算$\mathbf{H}_F^{-1}$，可以直接使用 Cholesky decomposition 结果

2. 无需使用$\mathbf{H}^{-1}$，可直接使用 Cholesky decomposition 的上三角矩阵作为替代
